...
export const GRASS_VIEW = {
    ...
    upgrades: {
        tillering: {
            title: "Tillering",
            description: "Every tile the grass takes for itself is worth more Growth.",
            max: 4,
            cost: (s, lvl) => ({ growth: D(200).mul(D(4).pow(lvl)) }),
        },
        seedBank: {
            title: "Seed Bank",
            description: "Grass spreads onto a new tile partly grown instead of as a bare seed.",
            max: 3,
            cost: (s, lvl) => ({ growth: D(400).mul(D(5).pow(lvl)) }),
        },
        greenFingers: {
            title: "Green Fingers",
            description: "Every grassy tile is worth more Green Essence, whichever type it is.",
            max: 3,
            cost: (s, lvl) => ({ growth: D(600).mul(D(6).pow(lvl)) }),
        },
        hardyStrains: {
            title: "Hardy Strains",
            description: "Grasses give back some of whatever they trade away. Does nothing for what they trade it for.",
            max: 3,
